...
class WLD implements CryptoPlugin {
  public depositUrl (address: string, amount: string): string {
    return `ethereum:pay-0x2cFc85d8E48F8EAB294be644d9E25C3030863003@480/transfer?address=${address}&uint256=${amount}e18`
  }

  public parseUrl (network: string, uri: string, opts?: any): string | never {
    const cryptoCode = (opts && opts.cryptoCode) || 'WLD'
    try {
      ...
      if (rec.protocol === 'ethereum:' && cryptoCode === 'WLD' && queryAddress) {
        throw new Error('Invalid Address')
      }

      if (rec.protocol === 'ethereum:' && cryptoCode === 'WLD') {
